...
// ————— ٤ب) **المرشدُ الرحّالة — البند الثامن** (حكمُ المالك، ١٦ أغسطس ليلاً) —————
//
// «الطائرُ الرحّالة بقبعة» (المرشح أ) حلّ محلَّ عصفور البذرة النائب، **ويختم جوازَ
// الطفل عند البوابات**. وثلاثةُ أبوابٍ تحرسه، وكلُّها تقيس **ما يُرسَم** لا ما يُقال
// في تعليق:
//   ١) **لا نيابةَ باقية**: لا يُعلَن في `ui.js` مرشدٌ «نائبٌ» ولا «مرفوعٌ» بعد الحكم.
//   ٢) **القبعةُ في الرسم**: هي فارقُ المرشح أ عن عصفور اقرأ — ورسمُها بلون مسار
//      الحرف من اللوح لا بلونٍ مكتوبٍ بيد (فيتبع الثيمَ الليليّ بلا نسخةٍ ثانية).
//   ٣) **والجوازُ يُختم فعلاً**: `gate.js` يستدعي `passportStamp` في احتفال العبور
//      — فالاستعارةُ عاملةٌ لا موصوفة، **ولا نصَّ منطوقاً جديداً** يدخل بها.
const ui = readFileSync(new URL('app/js/ui.js', ROOT), 'utf8');
const gate = readFileSync(new URL('app/js/gate.js', ROOT), 'utf8');
const mascotSvg = (ui.match(/const MASCOT_SVG = `([\s\S]*?)`;/) || [])[1] || '';

// **والنيابةُ تُقاس حيث تُعلَن**: في كتلة المرشد نفسِها (من عنوانها إلى دالّتها) —
// لا في الملفّ كلِّه، فذكرُ النيابة **في تاريخ** موضعٍ آخر أثرُ قرارٍ لا بقيّةٌ منه.
const mascotBlock = ui.slice(ui.indexOf('// الشخصية المرشدة'),
  ui.indexOf('export function mascot'));
ok(!!mascotSvg && /الطائرُ الرحّالة/.test(mascotBlock)
  && !/نائب|مرفوع|عصفورُ «اِقْرَأْ» نفسُه/.test(mascotBlock),
  'المرشدُ **الطائرُ الرحّالة** لا نائبَ البذرة (وسِمُ النيابة منزوعٌ من موضع إعلانه)');
ok(/rect[^>]*fill="var\(--accent-letter\)"/.test(mascotSvg)
  && (mascotSvg.match(/var\(--/g) || []).length >= 4
  && !/#[0-9a-f]{3,6}/i.test(mascotSvg),
  'وقبعتُه مرسومةٌ بألوان اللوح لا بلونٍ مكتوبٍ بيد (فيتبع الثيمَ الليليّ)');
ok(/export function passportStamp/.test(ui) && /passportStamp\(gate\.face\)/.test(gate)
  && /خُتِمَ جَوَازُكْ/.test(gate),
  '**ويختم جوازَ الطفل في احتفال البوابة** — الاستعارةُ عاملةٌ لا موصوفة');
// **ولا نصَّ منطوقاً جديداً بالختم** (عهدُ `gate.js` في رأسه): الختمُ صورةٌ، وسطرُه
// المكتوب له ملفُّه في البنك سلفاً — يُقاس بجرد الفهرس لا بالثقة.
const bank = JSON.parse(readFileSync(new URL('app/audio/manifest.json', ROOT), 'utf8'));
ok(Object.values(bank).includes('خُتِمَ جَوَازُكْ'),
  'وسطرُ الختم له ملفُّه في البنك (فلا يُضيف الختمُ نصّاً منطوقاً جديداً)');
