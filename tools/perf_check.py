...
**منقولةٌ من «اِقْرَأْ»** (الجلسة ٩) بعلّتها نفسِها: **لا يُصلَح ما لم يُقَس**، وجمهورُنا
أصغرُ سنّاً وأجهزتُه هي هي — فتدخل العدّةُ قبل أن يجيء بلاغُ بطءٍ لا نعرف أين موضعه.

**ولماذا نصّاً لا صورة**: لقطةُ Chrome بلا رأسٍ تخرج بيضاء متى صُوّرت قبل الطلاء.
فالصفحةُ في وضع `#auto` تُرسل أرقامَها إلى `/__perf` وهذه العدّةُ تطبعها — كما يفعل
`browser_test.py` سواءً بسواء.
...
def measure(port: int, timeout: int) -> int:
    ...
    if not Path(CHROME).exists():
        ...
        return 1
    server = serve(port)
    profile = tempfile.mkdtemp(prefix='listen-perf-')
