...
ok(/caches\.delete/.test(sw) && /SHELL_CACHE = `listen-shell-\$\{VERSION\}`/.test(sw),
  'ورفع النسخة يمحو مخزون **القشرة** القديم (لا يعلَق طفل على نسخة قديمة)');

// **وسابقةُ ما قبل الهوية تُكنَس** (جلسةُ الهوية هـ): هُوجرت `english-*` إلى
// `listen-*`، فمخزونُ القشرة القديم يبقى على أجهزة التطوير وزناً ميتاً إن لم يُكنَس —
// ويُقاس **بالسابقتين معاً في جرد الكنس** لا بذكرِ اسمٍ في تعليق.
ok(/const OURS = \[[^\]]*'listen-'[^\]]*'english-'[^\]]*\]/.test(sw)
  && /OURS\.some\(\(p\) => n\.startsWith\(p\)\)/.test(sw),
  'ويكنس معها سابقةَ `english-*` اليتيمة (هجرةُ المفاتيح يوم الهوية)');
