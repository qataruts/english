...
def main() -> int:
    ...
    # **ورفعتها الجلسةُ ع إلى ١٠٢٠٠** تبعاً لمهلة الصفحة (٩٦٠٠): قاعدةُ الكفاية
    # أطالت كلَّ محطةٍ تُساق (جولاتُ «وحدك» صارت دالّةَ مفاتيحها)، والجولةُ تُنطَق
    # بصوتٍ حقيقيّ فثمنُها ثوانٍ — والعلّةُ والقياسُ مكتوبان في `browser_test.html`
    # حيث الرقمُ الحاكم.
    ap.add_argument("--timeout", type=int, default=10200, help="ثوانٍ قبل الاستسلام")
