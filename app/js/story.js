 ...
 * **تُسمَع بلسانها قبل أن يُسأل عنها**. **ولا يسقط السطرُ إلا لعيبِ بيانات** (كلمةٌ
 * خارج المنهج أصلاً — يحرسه `check_range` في النصّ نفسِه).
 *
 * ————— **والسَّندُ ضاق إلى موضعه: الشائكاتُ وحدَهنّ** (الجلسة ع) —————
 *
 * **وعلّتُه**: الشائكةُ **تُدرَّس كلُّها ولا تُفكّ** (`METHOD.md §١٢-١`: «مفكوكةٌ إلا
 * موضعَ شوكتها»)، فمن لم يلقَها سمعاً لا سبيل له إليها بالفكّ — فيُسمعها المعلّم.
 * **أمّا كلمةُ الفكّ فالطفلةُ تفكّها بنفسها**، وقد صارت مدرَّبةً عليها: مفاتيحُ محطتها
 * السمعية تُصاب مرّتين في جلستها ثم تُنسَج فيما بعدها (قاعدةُ الكفاية —
 * `station.js`). **ونطقُها قبل السؤال يُبطل التمرين نفسَه**: مَن سمع `dog` ثم رأى
 * سطراً فيه `dog` لم يفكّ شيئاً، بل طابق صوتاً بصورة — وذاك درسٌ آخر.
 *
 * **وإن أوقف رفعُه حلقةً فذلك يعني أنّ التكرارَ لم يكفِ** — يُبلَّغ ولا يعود الحاجز
 * (مرسومُ المالك، `METHOD.md §١٢-١٤`). ويحرسه `tools/test_enough.mjs`.
 */
function lineSpecs(text, gradeId, isMastered) {
  ...
  for (const token of String(text).split(/\s+/).filter(Boolean)) {
    ...
    if (!spec) return null;
    const grown = ripe.has(token) || ripeTricky.has(token);
    // **والسَّندُ للشائكة وحدَها**: كلمةُ الفكّ تُفكّ ولو لم تنضج — فالفكُّ هو الدرس
    out.push(grown || spec.unit !== 'tricky' ? spec : { ...spec, support: true });
  }
  return out;
}
...
function pageRound(station, page, rnd, isMastered) {
  ...
  return {
    ...
    target: page.pick,
    line,
    /* **ما يُسنَد بصوته قبل السؤال**: **شائكاتُ** السطر التي لم تنضج سمعاً — تُنطَق
       بلسانها فيلقاها الطفلُ بمعناها، ثم يُسأل. وكلماتُ الفكّ ليست منها (`lineSpecs`). */
 ...
 * المشتّتاتِ وحدَها.
 *
 * **ولا تدريبَ يُزاد ولا نسجَ يُدَسّ هنا** (قاعدةُ الكفاية — `station.js`): مفتاحُ
 * القصة واحدٌ (`text|<درجة>|read`) تُصيبه صفحاتُها «وحدك» مرّتين فأكثرَ أصلاً (أربعُ
 * صفحاتٍ فصاعداً)، **وزيادةُ جولةٍ فيها إعادةُ صفحةٍ بعينها أو دسُّ تمرينٍ في وسط
 * نصٍّ يُتابَع** — وكلاهما ينقض الحكمَ الأول في رأس هذا الملفّ. والتدريبُ والنسجُ
 * موضعُهما محطاتُ الدرجة التي تسبقها.
 */
