...
// ————— **قاعدةُ الكفاية: المحطةُ تُدرِّب حتى تُعلِّم** (بلاغُ الميدان ٦) —————
//
// **العلّةُ مقيسة، ولفظُ المالك سمّاها**: «فتح الان ولكن الطفل غير مجهز، لا كلمات
// ولا حروف مكتملة ولم يتدرب على شيئ ثم نريده ان يقرأ — من صور واصوات الى قراءة
// جمل. ما هذا التدريب؟». والقياسُ يشهد: قبل أوّل قصةٍ **ستٌّ وثلاثون محطةً تُدرِّس
// إحدى وعشرين كلمةً وسبعةً وعشرين رمزاً** — فالمادّةُ موجودة. **لكنّ جولات «وحدك»
// كانت خمساً ثابتة** مهما كانت مفاتيحُ المحطة: محطةُ الحيوانات تسعةَ عشرَ مفتاحاً،
// فتُسأل عن خمسةٍ ويمضي أربعةَ عشرَ بلا سؤالٍ واحد — **وكلُّ مفتاحٍ يُقاس مرّةً
// واحدة**، والمحطةُ تُعطي نجومَها من أوّل مرور فتنفتح التالية. فالرحلةُ ثلاثةٌ
// وعشرون يوماً، والطفلةُ ترى الكلمةَ مرّةً ثم يُطلَب منها أن تقرأها في جملة —
// **ومرّةٌ واحدة ليست تدريباً**.
//
// **وقرارُ المالك (٢٤ أغسطس ٢٠٢٦): «لا مانع من اطالة المنهج»** — فالعلاجُ حركتان
// في صلب الطريق لا حاجزاً ولا بطاقةً جانبية، وهما ما يفعله هذا الموضع:
//
//   ١) **كلُّ مفتاحٍ من مفاتيح المحطة يُصاب `DRILL_TOUCHES` مرّاتٍ في جلستها** —
//      فعددُ جولات «وحدك» **دالّةُ مفاتيحها** لا ثابتٌ أعمى.
//   ٢) **وكلُّ جلسةِ محطةٍ تنسج `WEAVE_ITEMS` عناصرَ من محطاتٍ سابقة** بالأضعف أوّلاً
//      من ليتنر — **فالتكرارُ في الرحلة نفسِها لا في بطاقةٍ جانبية**، وتبقى
//      المراجعةُ اختياريةً كما أمر المالك (`METHOD.md §١٢-١٤`).
//
// **وسقفُ اليوم (١٥ دقيقة) يبقى على حاله**: محطةٌ ثقيلة تُستوفى في يومين أو ثلاثة
// متتابعة (`test_promise.mjs` يسعّرها بمحتواها — `METHOD.md §١٢-١٦`)، **ولا حبس**:
// المحطةُ تُعطي نجومَها متى أُتمّت، ولا عقدةَ تنتظر شيئاً.

/**
 * **كم مرّةً يُصاب المفتاحُ الواحد في جلسة محطته** — عتبةٌ معلَنة تُقرأ حيث تُطبَّق.
 * ودونها كان **مرّةً واحدة** (خمسُ جولاتٍ ثابتة موزَّعةٌ بالقرعة على مفاتيح المحطة
 * كلِّها)، وذاك ما قاسه بلاغُ الميدان ٦. ويحرسها `tools/test_enough.mjs` مُجرَّبةً
 * سالباً بخفضها إلى الواحدة.
 *
 * **وهي صندوقُ الإتقان نفسُه ولا رقمَ ثانٍ** (`progress.MASTERED_BOX`): «المحطةُ
 * تُدرِّب **حتى تُعلِّم**» تعني حرفاً أن يبلغ مفتاحُها في جلستها **العتبةَ التي
 * يقيس بها المنهجُ كلُّه الإتقان** — وهي عتبةُ قيد الاقتران (`METHOD.md §٦`) وعتبةُ
 * حارس الكفاية معاً. ولو نقصت عنها لَبقي بلوغُها معلَّقاً على **مصادفة** المراجعة
 * (قِيس: كلمةٌ في قصة ح٨ بلغت لمستين لا ثلاثاً لأنّ محطتها السمعية سبقت قصّتها
 * بيومٍ واحد) — **والمصادفةُ هي عينُ ما نقضه بلاغُ الميدان ٥**.
 */
export const DRILL_TOUCHES = progress.MASTERED_BOX;

/** **ما تنسجه جلسةُ المحطة من محطاتٍ سابقة** — بالأضعف أوّلاً، وأقلُّ منه إن قلَّت الحصيلة. */
export const WEAVE_ITEMS = 3;

/**
 * **أرضيةُ جولات «وحدك»** — وهي العددُ القائم قبل اليوم (خمس)، فلا تُقصِّر قاعدةُ
 * الكفاية جلسةَ محطةٍ قليلةِ المفاتيح: محطةُ تمييزٍ بزوجٍ واحد كانت تُدرّبه خمساً،
 * فلو اكتُفي فيها بنصيب المفتاح الواحد لَصار «التدريبُ حتى التعليم» **نقصاً** حيث
 * أُريد زيادة.
 * فما دون الأرضية يُتَمّ بدورةٍ أخرى على المفاتيح نفسِها (`METHOD.md §٤`: حلقةُ
 * المحطة ٣–٥ دقائق).
 */
export const MIN_SOLO = 5;

/**
 * **سقفُ جولاتِ الزيارة الواحدة — القضمة** (بلاغُ الميدان ٧، ٢٤ أغسطس ٢٠٢٦).
 *
 * **العلّةُ ميدانيةٌ حابسة**: قاعدةُ الكفاية جعلت جولاتِ المحطة دالّةَ مفاتيحها، فبلغت
 * ح١٣ **أربعاً وخمسين دقيقة**؛ **ولا استئنافَ داخل المحطة** — فمن خرج في وسطها عاد
 * إلى أوّلها، فلا يبلغ آخرَها أبداً. **وذلك جدارٌ جديد** ينقض مرسومَ المالك («لا توقف
 * في الحلقات اطلاقاً») أشدَّ ممّا نقضه الأول.
 *
 * **والعلاجُ قضمةٌ لا حفظُ حال**: الزيارةُ تأخذ بقدر يومها، **والمحطةُ تُزار مراتٍ**
 * — وهو مقصودُ التكرار أصلاً. **ولا حالةَ جلسةٍ تُخزَّن ولا استئنافٌ هشّ**: نصيبُ
 * المفتاح الباقي يُقرأ من **سجلّ ليتنر نفسِه** (‏`touchesOf`)، فما قِيس لا يُعاد،
 * وما بقي يُستكمَل — **والعودةُ تبدأ من حيث انتهت بحكم البيانات لا بحكم ذاكرة**.
 *
 * **ورقمُه مشتقٌّ من سقف اليوم لا مختار** (`METHOD.md §١٢-٨`: خمسَ عشرةَ دقيقة، وتسعيرُ
 * `test_promise`: زيارةٌ بدقيقة وجولةٌ برُبعها وتدريسُ مفتاحٍ بـ٠٫١٥): ثلاثون جولةً
 * ≈ ثمانيَ دقائقَ ونصفاً، **فيبقى للتدريس وللمنسوج فُسحةٌ تحت السقف** ولا تطول جلسةُ
 * طفلٍ في السادسة.
 */
export const VISIT_ROUNDS = 30;

/**
 * **كم مرّةً قِيس هذا المفتاحُ قبل اليوم** — إصاباتُه وأخطاؤه في سجلّ ليتنر (`right`
 * و`wrong`): كلُّ قياسٍ لمسةُ تدريبٍ سواءٌ أصاب أم أخطأ (والخطأُ يُعالَج في حينه —
 * عهدُ «لا شاشة خطأ»). **وهو ما يجعل القضمةَ تُستأنف بلا ذاكرةِ جلسة.**
 */
const touchesOf = (records, key) => {
  const rec = records.get(key);
  return rec ? (rec.right || 0) + (rec.wrong || 0) : 0;
};

/** **أبلغت مفاتيحُ هذه المحطة نصيبَها من التدريب؟** — يقرؤه الحارسُ والخريطةُ سواء. */
export const isDrilled = (station, records = new Map(progress.skills().map((s) => [s.key, s]))) =>
  (station.skills || []).every((key) => touchesOf(records, key) >= DRILL_TOUCHES)
  || exhausted.has(station.id);

/* **ومحطةٌ أعطت كلَّ ما عندها لا تُحبَس على ما ليس دَينَها**: مفتاحٌ لا مادّةَ له
   اليوم (كلمةٌ محطتُها السمعية بعدُ — قيدُ الاقتران) لا تستطيع محطتُه تدريبَه مهما
   زِيرت، **ويقيسه `test_promise` في مجرى الأيام**. فحين تنتهي زيارةٌ ولم يبقَ في
   المحطة مفتاحٌ **ناقصٌ وله مادّة**، تُعلَن «أعطت ما عندها» فيمضي السهم — **بابٌ لا
   جدار**، وهو نظيرُ ما تفعله المراجعةُ حين لا تجد تمرينَ مهارة. */
const exhausted = new Set();
export const markExhausted = (stationId) => exhausted.add(stationId);

/**
 * **مفاتيحُ ما تكتبه جولةٌ — تُقرأ من كاتبها هو** لا تُستنتَج من حقلَيها: جولةُ
 * الأمر المركّب تكتب صفتين (`tpr-two`)، فلو عددناها مفتاحاً لَحسبنا نصفَ تدريبها.
 * وهي الدالّةُ نفسُها التي يقرأ بها حارسُ الوعد ما تكتبه الشاشة (`scoreOf`).
 */
export function keysWritten(round) {
  const out = [];
  const write = (unit, range, kind) => out.push(`${unit}|${range}|${kind}`);
  const score = round?.kind && scoreOf(round.kind, round.by ?? 0);
  if (score) score(round, true, write);
  else if (round?.unit && round?.kind) write(round.unit, round.range, round.kind);
  return out;
}

/** مفتاحٌ نصّاً إلى أجزائه — بقاعدة `progress.spanOf` الواحدة. */
const skillOfKey = (key) => {
  const [unit, range, kind] = key.split('|');
  return { key, unit, range, kind };
};

/**
 * **جولاتُ التدريب**: تُبنى جولةٌ لكلِّ مفتاحٍ من مفاتيح المحطة، **ثم تُعاد الكرّةُ**
 * حتى يبلغ كلُّ مفتاحٍ نصيبَه — والدورةُ الثانية بترتيبٍ آخر، **فلا تتكرّر الجولةُ
 * نفسُها حرفاً في إثر نفسِها** بل يفصل بينهما دورٌ كامل.
 *
 * **ومفتاحٌ لا مادّةَ له اليوم يُترَك ولا يوقف شيئاً** (`makeRound` تردّ `null`:
 * كلمةٌ لم تنضج سمعاً، أو شكلٌ لا يجد مشتّتَه) — **بابٌ لا جدار**، وهو عينُ ما
 * تفعله المراجعةُ حين لا تجد تمرينَ مهارة.
 *
 * @param {object} station المحطةُ بمفاتيحها المعلَنة (`skills`)
 * @param {Function} rnd مولّدُ الصدفة المبذور
 * @param {(skill: object) => object|null} makeRound بانِي جولةٍ لمفتاحٍ بعينه — **من
 *   الوحدة المالكة**: هي التي تعرف كيف يُترجَم المدى إلى مادّةِ جولتها.
 * @param {object[]} extra جولاتٌ تزيدها الوحدةُ بعد التدريب (زوجُ العناية مثلاً)
 */
export function drillRounds(station, rnd, makeRound, extra = [], touches = DRILL_TOUCHES) {
  /* **ونصيبُ المفتاح ما بقي منه لا ما كان** (القضمة — `VISIT_ROUNDS`): ما قِيس في
     زيارةٍ سابقة أو نُسج في محطةٍ تالية **يُحسَب**، فلا يُعاد تدريبُ ما تدرَّب،
     وتُستأنف المحطةُ من حيث انتهت بحكم السجلّ لا بحكم ذاكرةِ جلسة. */
  const records = new Map(progress.skills().map((skill) => [skill.key, skill]));
  const need = new Map((station.skills || [])
    .map((key) => [key, Math.max(0, touches - touchesOf(records, key))]));
  const out = [];
  const count = (round) => {
    for (const key of keysWritten(round)) {
      if (need.has(key)) need.set(key, need.get(key) - 1);
    }
  };
  const live = new Set(need.keys());        // مفاتيحُ لها مادّةٌ اليوم
  for (const round of extra) count(round);
  const pass = (keys) => {
    for (const key of shuffle(keys, rnd)) {
      // **والقضمةُ تقف عند سقفها** — وما بقي تأخذه الزيارةُ التالية
      if (out.length + extra.length >= VISIT_ROUNDS) return;
      const round = makeRound(skillOfKey(key));
      // **ومفتاحٌ بلا مادّةٍ اليوم يُترَك ولا يُعاد سؤالُه** — بابٌ لا جدار
      if (!round) { need.set(key, 0); live.delete(key); continue; }
      out.push(round);
      count(round);
    }
  };
  for (let at = 0; at < touches; at++) {
    pass([...need.keys()].filter((key) => need.get(key) > 0 && live.has(key)));
  }
  /* **وإن لم يبقَ ناقصٌ له مادّة فقد أعطت المحطةُ ما عندها** — يُعلَن ههنا حيث
     يُعرَف (‏`live` جردُ ما وجد مادّتَه اليوم)، لا يُستنتَج في الخريطة. */
  if (![...need.keys()].some((key) => need.get(key) > 0 && live.has(key))) {
    markExhausted(station.id);
  }
  /* **ثم تُبلَغ الأرضيةُ بدورةٍ أخرى** (`MIN_SOLO`): محطةُ مفتاحٍ واحد تُدرّبه خمساً
     كما كانت، ولا تنقص جلسةٌ عمّا كانت عليه قبل قاعدة الكفاية. */
  while (out.length + extra.length < MIN_SOLO && live.size
    && out.length + extra.length < VISIT_ROUNDS) {
...
/**
 * **نسجُ السابق في اللاحق**: عناصرُ من محطاتٍ **أتمّها الطفلُ فعلاً**، بالأضعف أوّلاً
 * من ليتنر — تُبنى ببانيها هي (`itemFor`) وتُرسَم بمُصيِّره هو (`woven`).
 *
 * **وعلّتُها مقيسة**: بلاغُ الميدان ٥ قاس أنّ المراجعةَ العامّة لا تصادف مفاتيحَ
 * عقدةٍ بعينها (صفرُ تمرينٍ في عشرة أيام)، فلا يجوز أن يُعوَّل عليها في التكرار —
 * **والمراجعةُ بطاقةٌ اختيارية بمرسوم المالك**. فصار التكرارُ في الرحلة نفسِها.
 *
 * ————— **وجبهةُ المنسوج جبهتُه هو لا جبهةُ مضيفه** —————
 *
 * جبهاتُنا **ليست تراكمية عبر الجبهتين**: جبهةُ درجةِ حرفٍ حقولُها فارغة (نصٌّ لا
 * صور)، وجبهةُ محطة تمييزٍ رموزُها فارغة. فلو حُوكمت الجولةُ المنسوجة بجبهة المحطة
 * **المضيفة** لَبدت خارجةً عنها وهي ليست كذلك.
 *
 * **والحكمُ الصحيح أنّها تُحاكَم بجبهة صاحبها**: هي جولةُ **محطةٍ أتمّها الطفل**،
 * بناها بانيها هو من مادّة تلك المحطة بعينها — و`check_range` قد جرد تلك المحطةَ
 * بأربع بذورٍ تحت جبهتها. **فلا مادّةَ تفلت من الجرد**، وإنّما تُقابَل بجبهتها هي.
 * وهو **عينُ ما تفعله جلسةُ المراجعة** منذ يومها الأول (تخلط الجبهتين في جلسةٍ
 * واحدة، ولا تُجرَد بجبهةِ محطةٍ لأنّها ليست محطة) — والنسجُ **نقلُ مادّة المراجعة
 * إلى الرحلة** لا صنفٌ جديد من العرض. **وقيدُ الاقتران يبقى الحاكم** فيها كما في
 * المراجعة: البانِي يمرّ من `readableAt` نفسِها، ويشهد `test_promise` على الرحلة
 * الحيّة (صفرُ خرقٍ في ستّين يوماً والنسجُ عامل).
 */
export function wovenRounds(station, rnd, count = WEAVE_ITEMS) {
  const own = new Set(station.skills || []);
  const past = new Set();
  for (const one of stations()) {
    if (one.id === station.id || !progress.isDone(one.id)) continue;
    for (const key of one.skills || []) past.add(key);
  }
  if (!past.size) return [];
  const out = [];
  for (const skill of progress.weakestSkills()) {
    if (out.length >= count) break;
    if (!past.has(skill.key) || own.has(skill.key)) continue;
    const round = itemFor(skill, rnd);
    if (round) out.push({ ...round, woven: true });
  }
  return out;
}

/** نسجٌ **داخل الجلسة** لا في ذيلها: توزيعٌ متباعد، فيلقى الطفلُ السابقَ في أثناء رحلته. */
function interleave(rounds, woven) {
  if (!woven.length) return rounds;
  const out = [...rounds];
  const step = Math.max(1, Math.floor(rounds.length / (woven.length + 1)));
  woven.forEach((round, at) => {
    out.splice(Math.min(out.length, step * (at + 1) + at), 0, round);
  });
  return out;
}

/**
 * **جولاتُ «وحدك» كما صارت**: تدريبُ مفاتيح المحطة (`drillRounds`) منسوجاً به ما
 * سبق (`wovenRounds`) — وهي الدالّةُ الواحدة التي تناديها وحداتُ التمارين كلُّها،
 * فلا تفترق محطةٌ عن أختها في قدر تدريبها.
 */
export function soloRounds(station, rnd, makeRound, extra = []) {
  return interleave(drillRounds(station, rnd, makeRound, extra), wovenRounds(station, rnd));
}
...
export function stationScreen({ nodeId, title, accent, make, view, score, save }) {
  ...
  const hooks = () => {
    ...
    return {
      ...
      attempt: (round, correct) => {
        if (!measured) return;
        // **والمنسوجُ يكتبه كاتبُه هو** (`wovenRounds`): جولةُ محطةٍ سابقة تكتب
        // مفاتيحَها بمُسجِّل وحدتها لا بمُسجِّل المحطة التي نُسجت فيها.
        (round.woven ? wovenScore : score)(round, correct);
  ...
  function paint(silence = true) {
    ...
    const round = phase.key === 'watch' ? null : plan[phase.key][state.index];
    body.replaceChildren(phase.key === 'watch'
      ? watchStep(plan.model, hooks())
      // **والمنسوجُ يرسمه مُصيِّرُه هو** لا مُصيِّرُ المحطة المضيفة
      : (round.woven ? wovenView : view)(round, hooks()));
  }
...
/**
 * **جولةٌ منسوجةٌ من محطةٍ سابقة — بمُصيِّر وحدتها ومُسجِّلها** (`wovenRounds`):
 * تحمل مالكَها في `by` كما تحمله جولةُ المراجعة، فتُرسَم وتُكتَب بمن بناها لا بمن
 * استضافها — ولولا ذلك لَرسمت شاشةُ الأصوات جولةَ تركيبٍ ولَكتبت مفتاحاً ليس لها.
 */
const wovenSpec = (round) => (EXERCISES.get(round.kind) || [])[round.by ?? 0] || null;

function wovenView(round, hooks) {
  const spec = wovenSpec(round);
  return spec ? spec.view(round, hooks) : h('p', { class: 'hint' }, 'لا تمرين بعد.');
}

function wovenScore(round, correct) {
  wovenSpec(round)?.score(round, correct, progress.recordAttempt);
}
