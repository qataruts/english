...
function renderMap() {
  ...
  /* **والسهمُ يشير إلى عقدةِ الإنضاج نفسِها** (بلاغُ الميدان ٥ — تصويبُ الإسعاف م):
     صُرف الوعدُ أوّلَ مرّةٍ إلى بطاقة المراجعة بحجّة أنّ عقدةَ الإنضاج «لا تُلعَب
     بعدُ» — **ونقضه الميدانُ والقياسُ معاً**: المراجعةُ العامة تنتقي بالاستحقاق من
     الحصيلة كلِّها، **فلا تخدم مفاتيحَ هذه العقدة إلا مصادفةً** (قِيس: عشرةُ أيامٍ
     متتابعة، صفرُ تمرينٍ من مفاتيحها الثمانية — فلا تنفتح أبداً). وجلسةُ الإنضاج
     تخدمها بأعيانها فتفتحها في ثلاث. **والحجّةُ الأولى سقطت بفعلِ الإصلاح نفسِه**:
     عقدةُ الإنضاج **تُلعَب اليوم** — لمستُها جلسةٌ تُنضجها. */
  if (next) {
  ...
  /* **بابُ المرجع التعريفيّ في ذيل الخريطة** (الجلسة ٩): رابطٌ هادئ إلى
     `welcome/` — للوالد والمعلّم يريدان المنهجَ وأسسَه، **ولا يلزم طفلاً فعلٌ
     وسيلتُه قراءتُه** (ذيلُ شاشةٍ لا زرُّ محطة، وقاعدةُ اللاقراءة تحرس ما يلزم
     الطفلَ لا ما يُعرَض للكبار). **ورابطٌ عاديّ لا مسارُ hash**: الصفحاتُ خارج
     التطبيق وخارجَ قشرة `sw` — يخرج إليها الجهازُ كما يخرج إلى أيّ صفحة. */
  main.append(h('a', { class: 'map-about', href: 'welcome/' },
    'عن التطبيق: المنهجُ وأسسُه — لوليّ الأمر والمعلّم'));
...
function nodeButton(node, next) {
  ...
  /* **وعقدةُ الإنضاج تحمل السهمَ ووسمَها معاً** (بلاغُ الميدان ٥): الرمليةُ تقول
     للوالد «مادّتُها تنمو»، والسهمُ يقول للطفل «من هنا» — ولا تعارض: لمستُها تعمل. */
  const isNext = Boolean(next) && next.id === node.id;
...
/* ————— إشعارُ التحديث الهادئ (بندُ الجلسة ٩ — درسُ «اِحْسِبْ» م١٠·أ) —————
 *
 * نسخةُ القشرة تُرى في لوحة وليّ الأمر (`METHOD.md §١٢-١٠`)، **وهذا شقُّها الثاني:
 * أن لا يُسأل السؤال أصلاً**. فقشرةٌ جديدة تتفعّل والصفحةُ مفتوحة تترك الجهازَ على
 * حالٍ ملتبس — شيفرةٌ قديمة تعمل في الذاكرة وجديدةٌ في المخزون تنتظر فتحةً — وثمنُه
 * دُفع مرّتين عند أخينا («لا يشهد ميدانٌ على شيفرة ميّتة»، ثم «هل لم يصل التحديث؟»).
 *
 * **وثلاثةُ قيودٍ لكلٍّ علّتُه:**
 *  ١) **لا يقطع تمريناً**: لا صوتَ ولا حوارَ ولا إعادةَ تحميلٍ تلقائية — والإشعارُ
 *     **لا يُعرَض على شاشة تمرينٍ أصلاً**: يُنتظَر به حتى يعود إلى الخريطة أو يفتح
 *     وليُّ الأمر لوحَه، فهو خبرُ كبارٍ لا مقاطعةُ درس.
 *  ٢) **لا يحجب شيئاً**: سطرٌ **في مجرى الصفحة** فوق الترويسة كشريط المعاينة، لا
 *     طبقةٌ عائمة تغطّي زرّاً تحتها.
 *  ٣) **ولا فعلَ فيه**: لا زرَّ ولا وسيطَ يلزم أحداً — فلا يقع تحت حارس اللاقراءة
 *     (وهو يحرس ما **يلزم الطفلَ** فعلُه)، ويُقرأ ويُترك.
 *
...
// والمعاينةُ للمعلّم يفحص المنهجَ لا لزائرٍ يُدعى إلى التثبيت.
