...
// الحرف والقصصُ والبوابات — فالبنيةُ محسومة، **والقيمُ نائبةٌ مؤقتة** حتى قرار
// الهوية ق٣ (`docs/SEED.md §٤`).
...
// ————— عناصر الهوية: **نائبةٌ مؤقتة** (`docs/SEED.md §٤`) —————
//
// **الاسمُ واللوحُ والأيقونةُ قرارُ مالكٍ معلَّق** (ق٣ · `PLAN.md §٩` · ميثاق العائلة
// §٩ · وسياسةُ التسمية: **اسمٌ إنكليزيٌّ صريح لا نقحرة**). فما دون ذلك **نائبٌ
// مؤقت منسوخٌ من البذرة يُعلن نيابتَه ولا يُجتهَد فيه**، ويُستبدل **قبل أول عرضٍ
// على طفل**. ولذلك يحمل رسمُ العلامة `data-pending` — فحارسُ الهوية يومَ يُكتب
// (`test_identity.mjs` عند احسب) يمسكه بالجرد لا بانتباهٍ يُرجى.

// الشخصية المرشدة — **عصفورُ «اِقْرَأْ» نفسُه، نائباً مؤقتاً**: ألوانه من متغيّرات
// اللوح فيتبع الثيمَ الليليّ بلا نسخةٍ ثانية.
const MASCOT_SVG = `
...
/** الشخصية المرشدة — تظهر في بدايات المحطات والاحتفالات. **نائبٌ مؤقت** (ق٣). */
...
/**
 * **اسمُ التطبيق — معلَّقٌ بقرار المالك** (ق٣): سياسةُ التسمية تنصّ على **اسمٍ
 * إنكليزيٍّ صريح لا نقحرة** (`read/ref/sonnet-con/naming.md`)، وهو ما لا تجتهد فيه
 * جلسةُ تنفيذ. فهذا **وصفٌ لا اسم**، ويُبدَّل يومَ الهوية — ولا يُكتب في موضعٍ آخر:
 * كلُّ من يعرضه يستورده من هنا، فلا يبقى بعد التبديل أثرٌ للنائب.
 */
export const BRAND = 'الإنكليزية';
/** **إعلانُ النيابة** — يقرؤه حارسُ الهوية والمراجعة، ولا يُرفَع إلا بقرار ق٣. */
export const BRAND_PENDING = true;

/** علامةُ التطبيق: الكلمةُ ومرشدُها — **ومعها إعلانُ نيابتها في DOM** (`data-pending`). */
